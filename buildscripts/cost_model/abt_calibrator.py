...
from cost_estimator import estimate, ExecutionStats
...
def collect_stats(config: AbtCalibratorConfig, database: DatabaseInstance,
    ...
    """Read measurements from database and extract statitics for the given ABT types."""
...
def parse_explain(explain: Mapping[str, any], abt_types: Sequence[str]):
    ...
    lt = physical_tree.build(explain['queryPlanner']['winningPlan']['optimizerPlan'])
    for abt_type in abt_types:
        abt_node = find_abt_node_by_type(lt, abt_type)
