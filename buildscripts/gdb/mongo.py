...
def get_wt_session(recovery_unit, recovery_unit_impl_type):
    ...
    return (
        wt_session_handle.dereference().cast(
            gdb.lookup_type("mongo::WiredTigerSession")
        )["_session"]
        if wt_session_handle.dereference().address
        else None
    )
...
def get_decorations(obj):
    ...
    for i in range(count):
        ...
        try:
            ...
            print(f"Failed to look up decoration type: {type_name}: {str(err)}")
...
class MongoDBDumpRecoveryUnits(gdb.Command):
    ...
    def dump_recovery_units(recovery_unit_impl_type):    # pylint: disable=too-many-locals
        ...
        for client_handle in client_set:
            ...
            if wt_session := get_wt_session(
                recovery_unit, recovery_unit_impl_type
            ):
    ...
    def dump_session(session, recovery_unit_impl_type):
        ...
        output_doc["recoveryUnit"] = hex(recovery_unit_handle) if recovery_unit else "0x0"
        if wt_session := get_wt_session(recovery_unit, recovery_unit_impl_type):
            output_doc["WT_SESSION"] = hex(wt_session)
...
class MongoDBDumpStorageEngineInfo(gdb.Command):
    ...
    def invoke(self, arg, _from_tty):    # pylint: disable=unused-argument
        ...
        if main_binary_name == 'mongod':
            ...
            print(f"Not invoking mongod storage engine info dump for: {main_binary_name}")
