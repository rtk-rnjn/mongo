...
class _CppHeaderFileWriter(_CppFileWriterBase):
    ...
    def gen_enum_functions(self, idl_enum):
        ...
        if extra_data_decl is not None:
            self._writer.write_line(f"{extra_data_decl};")
...
class _CppSourceFileWriter(_CppFileWriterBase):
    ...
    def _gen_array_deserializer(self, field, bson_element, ast_type):
        ...
        if field.validator:
            self._writer.write_line(f'{_get_field_member_validator_name(field)}(values);')

        if field.chained_struct_field:
            if field.type.is_variant:
                self._writer.write_line(
                    f'{_get_field_member_name(field.chained_struct_field)}.{_get_field_member_setter_name(field)}({field.type.cpp_type}(std::move(values)));'
                )
    ...
    def gen_bson_deserializer_methods(self, struct):
        ...
        with self._block('%s {' % (func_def), '}'):
            ...
            if struct.cpp_validator_func is not None:
                self._writer.write_line(f"{struct.cpp_validator_func}(this);")
        ...
        with self._with_template(template_params):
            with self._block('stdx::visit(OverloadedVisitor{', '}, ${access_member});'):
                for variant_type in itertools.chain(
                    ...
                    template_params['cpp_type'] = (
                        f'std::vector<{variant_type.cpp_type}>'
    ...
    def _gen_server_parameter_specialized(self, param):
        ...
        self._writer.write_line(
            f'auto sp = makeServerParameter<{param.cpp_class.name}>({_encaps(param.name)}, {param.set_at});'
        )
